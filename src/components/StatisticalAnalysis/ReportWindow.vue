	...
	<div class="main">
		<div class="block">
			...
			<div class="value">
				...
				<input type="text" placeholder="--请选择处罚日期--" class="myinput" :disabled="disabledFlag" v-model="punishTime" readonly="readonly"
				    id="surface1">
			</div>
		</div>
		<div class="block">
			...
			<div class="value">
				<el-select filterable placeholder="--请选择--" :disabled="disabledFlag" v-model="carStyle">
		...
		<div class="block">
			...
			<div class="value">
				<input type="text" class="myinput" :disabled="disabledFlag" v-model="carNumber">
			</div>
		</div>
		<div class="block">
			<div class="name">
			...
			<div class="value">
				<el-select filterable placeholder="--请选择--" :disabled="disabledFlag" v-model="burnType">
		...
		<div class="block">
			...
			<div class="value">
				<el-select filterable placeholder="--请选择--" :disabled="disabledFlag" v-model="punishPlace">
					<el-option v-for="item in cfdd" :key="item.value" :label="item.value" :value="item.value">
					</el-option>
				</el-select>
			</div>
		</div>
		...
		methods: {
			...
			saves() {
				...
				if (submitFlag) {
					this.$http({
						method: "post",
						url: this.baseurl + "formProducInfo/save",
						data: {
			...
			getDetailData() {
				...
				if (id !== 'save') { // 不是新增 
					...
					this.$http({
							method: 'get',
							url: `${this.baseurl}formProducInfo/data/${id}`
						})
						.then(function (res) {
							if (res.status === 200 && res.data.status === '1') {
								...
								(_this.carStyle = _this.details.carStyle),
								(_this.burnType = _this.details.burnType),
